...
/**
 * @brief Initializes the DS3231 module. Set clock halt bit to 0 to start
 * timing.
...
/**
 * @brief Set the interrupt mode to either alarm interrupt or square wave
 * interrupt.
 * @param mode Interrupt mode to set, DS3231_ALARM_INTERRUPT or
 ...
 * @param rate Frequency to set, DS3231_1HZ, DS3231_1024HZ, DS3231_4096HZ or
 * DS3231_8192HZ.
 */
...
/**
 * @brief Clears alarm 2 matched flag. Matched flags must be cleared before the
 * next match or the next interrupt will be masked.
 */
...
/**
 * @brief Set alarm 2 date. Alarm 2 can only be set to match either date or day.
 * Does not change alarm 2 matching mode.
...
/**
 * @brief Set alarm 2 day. Alarm 2 can only be set to match either date or day.
 * Does not change alarm 2 matching mode.
 ...
 * @param alarmMode Alarm 2 mode, DS3231_A2_EVERY_M, DS3231_A2_MATCH_M,
 * DS3231_A2_MATCH_M_H, DS3231_A2_MATCH_M_H_DATE or DS3231_A2_MATCH_M_H_DAY.
 */
void DS3231_SetAlarm2Mode(DS3231_Alarm2Mode alarmMode) {
  ...
  DS3231_SetRegByte(
      DS3231_A2_DATE,
      temp | (((alarmMode >> 2) & 0x01) << DS3231_AXMY) | (alarmMode & 0x80));
}
...
/**
 * @brief Clears alarm 1 matched flag. Matched flags must be cleared before the
 * next match or the next interrupt will be masked.
 */
...
/**
 * @brief Set alarm 1 date. Alarm 1 can only be set to match either date or day.
 * Does not change alarm 1 matching mode.
 * @param date Date, 0 to 31.
 */
...
/**
 * @brief Set alarm 1 day. Alarm 1 can only be set to match either date or day.
 * Does not change alarm 1 matching mode.
 * @param day Days since last Sunday, 1 to 7.
 */
 ...
 * @param alarmMode Alarm 1 mode, DS3231_A1_EVERY_S, DS3231_A1_MATCH_S,
 * DS3231_A1_MATCH_S_M, DS3231_A1_MATCH_S_M_H, DS3231_A1_MATCH_S_M_H_DATE or
...
void DS3231_SetAlarm1Mode(DS3231_Alarm1Mode alarmMode) {
  ...
  DS3231_SetRegByte(
      DS3231_A1_DATE,
      temp | (((alarmMode >> 3) & 0x01) << DS3231_AXMY) | (alarmMode & 0x80));
}
...
uint16_t DS3231_GetYear(void) {
  ...
  uint16_t century =
      (DS3231_GetRegByte(DS3231_REG_MONTH) >> DS3231_CENTURY) * 100 + 2000;
 ...
 * @param year Year, 2000 to 2199.
 */
void DS3231_SetFullDate(uint8_t date, uint8_t month, uint8_t dow,
                        uint16_t year) {
  DS3231_SetDate(date);
...
/**
 * @brief Encodes a decimal number to binaty-coded decimal for storage in
 * registers.
 * @param dec Decimal number to encode.
 * @return Encoded binary-coded decimal value.
 */
uint8_t DS3231_EncodeBCD(uint8_t dec) { return (dec % 10 + ((dec / 10) << 4)); }
